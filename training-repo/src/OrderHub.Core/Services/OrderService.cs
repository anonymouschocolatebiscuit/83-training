...
public class OrderService : IOrderService
{
    ...
    public async Task<ServiceResult<Order>> CreateOrderAsync(int customerId, IReadOnlyList<NewOrderLine> lines)
    {
        ...
        var requestError = ValidateOrderRequest(customer, lines);
        ...
        var buildResult = await BuildOrderItemsAsync(order, lines);
    ...
    // 訂單層級的請求驗證（客戶存在、明細非空、數量為正、商品不重複）。
    // 通過回傳 null，否則回傳第一個錯誤訊息。
    ...
    // 逐項驗證商品（存在/販售中/庫存足夠），通過就扣庫存並加入明細（快照存原價）。
    // 任一項失敗即彙整所有錯誤回傳，呼叫端不會存檔。
    private async Task<ServiceResult<Order>> BuildOrderItemsAsync(Order order, IReadOnlyList<NewOrderLine> lines)
    {
        ...
        foreach (var line in lines)
        {
            ...
            // 快照存「當下原價」；會員折扣一律集中在 CalculateTotal 算一次，不在此重算。
    ...
    public async Task<ServiceResult<Order>> CancelOrderAsync(int id)
    {
        ...
        // 先把庫存加回（上方 guard 已保證此時只有 Pending / Confirmed），再把訂單設為已取消。
